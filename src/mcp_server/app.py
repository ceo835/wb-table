...
WB_DAILY_OPERATIONAL_SUMMARY_CONTENT_HINT = (
    ...
    "Не придумывай показатели и не используй fake/mock данные. Используй только предоставленные факты. Не утверждай причинно-следственные связи без подтверждения (вместо 'вызвано' пиши 'снижение совпало с', 'произошло на фоне'; вместо 'товар плохо продается из-за цены' пиши 'требуется проверить цену').\n"
    ...
    "При наличии weekly_analysis бери показатели изменения за неделю (оборот, заказы, рекламный расход, операционную прибыль) и классификацию тренда непосредственно из него.\n"
