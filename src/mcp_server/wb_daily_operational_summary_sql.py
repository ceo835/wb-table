...
def fetch_mart_daily_overview(
    ...
    sql = text(
        ...
        select
            ...
            sum(coalesce(impressions, entry_impressions_total)) as impressions,
            sum(coalesce(card_clicks, entry_card_clicks_total)) as card_clicks,
            case when sum(coalesce(impressions, entry_impressions_total)) > 0 then sum(coalesce(card_clicks, entry_card_clicks_total)) / nullif(sum(coalesce(impressions, entry_impressions_total)), 0) * 100 end as ctr,
            sum(cart_count) as cart_count,
            case when sum(coalesce(card_clicks, entry_card_clicks_total)) > 0 then sum(cart_count) / nullif(sum(coalesce(card_clicks, entry_card_clicks_total)), 0) * 100 end as add_to_cart_conversion,
            ...
            sum(ad_cost_writeoff_total) as ad_writeoff_total,
            sum(ad_campaign_spend_total) as ad_campaign_spend_total,
            sum(ad_revenue) as ad_revenue_total,
            ...
            case when sum(ad_clicks_total) > 0 then sum(ad_campaign_spend_total) / nullif(sum(ad_clicks_total), 0) end as cpc,
            case when sum(ad_views_total) > 0 then sum(ad_campaign_spend_total) / nullif(sum(ad_views_total), 0) * 1000 end as cpm,
            case when sum(ad_orders_total) > 0 then sum(ad_campaign_spend_total) / nullif(sum(ad_orders_total), 0) end as cpo,
            case when sum(ad_atbs_total) > 0 then sum(ad_campaign_spend_total) / nullif(sum(ad_atbs_total), 0) end as cost_per_cart,
